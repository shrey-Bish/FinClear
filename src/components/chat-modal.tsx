...
export function ChatModal({
  ...
  const sid = useMemo(() => {
    ...
    if (typeof window !== "undefined") {
      const key = "__SowSmart_session"; const ex = (window as any)[key]
  ...
  useEffect(() => {
    ...
    window.addEventListener("SowSmart:chat:open", onOpen as any)
    return () => window.removeEventListener("SowSmart:chat:open", onOpen as any)
  }, [])
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 p-4 sm:items-center">
      <div className="w-full max-w-2xl overflow-hidden rounded-2xl bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b px-5 py-4">
          <div>
            <div className="text-[12px] font-semibold tracking-[0.18em] text-[#1B5E20]">SowSmart CHAT</div>
            <div className="text-sm text-[#1B5E20]/70">Ask anything about your benefits</div>
          </div>
          ...
          {messages.map(m => (
            <div key={m.id} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-[85%] rounded-2xl px-3 py-2 text-sm ${
                m.role === "user" ? "rounded-br-sm bg-[#2E7D32] text-white" :
                m.role === "error" ? "border border-red-200 bg-red-50 text-red-800" :
                "rounded-bl-sm bg-[#F3F4F6] text-[#1A1A1A]"}`}>
                {m.content}
                {m.role === "assistant" && m.provider &&
                  <div className="mt-1 text-[10px] text-[#2E7D32]/70">{m.provider === "gemini" ? "Powered by Gemini" : "Knowledge Base"}</div>}
              </div>
            </div>
          ))}
        ...
        <div className="flex items-end gap-2 border-t px-5 py-4">
          <textarea value={input} onChange={(e)=>setInput(e.target.value)} onKeyDown={onKeyDown} placeholder="Press Enter to send • Shift+Enter for a new line" className="min-h-[52px] max-h-[140px] w-full resize-y rounded-2xl border border-[#E5E7EB] px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-[#2E7D32]/20" />
          <button onClick={()=>void send()} disabled={sending || !input.trim()} className="flex items-center gap-2 rounded-2xl bg-[#2E7D32] px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">Send</button>
        </div>
      </div>
    </div>
  )
